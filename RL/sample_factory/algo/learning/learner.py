...
class LinearDecayScheduler(LearningRateScheduler):
    def __init__(self, cfg):
        ...
        self.linear_decay = LinearDecay([(0, cfg.learning_rate), (num_updates, cfg.learning_rate_final)])
        self.step = 0
...
class Learner(Configurable):
    ...
    def init(self) -> InitModelData:
        ...
        if self.cfg.gate == 1:
            params_m = [param for name, param in self.actor_critic.named_parameters() if 'core.core.m' in name]
            params_others = [param for name, param in self.actor_critic.named_parameters() if 'core.core.m' not in name]
            params = [{'params': params_m, 'lr': 1e-3},
                    {'params': params_others, 'lr': self.cfg.learning_rate}]
        else:
        ...
        for epoch in range(self.cfg.num_epochs):
            ...
            for batch_num in range(len(minibatches)):
                ...
                with timing.add_time("losses_postprocess"):
                    ...
                    if self.cfg.region == 1:
                        spike_loss = 0.01 * torch.mean(self.actor_critic.core.core.hidden_state**2)
                        weight_loss = 0.1 * torch.mean(F.relu(self.actor_critic.core.core.w_rnn)**2) if self.cfg.rnn_type == 'trnn' else 0
                        loss: Tensor = actor_loss + critic_loss + spike_loss + weight_loss
                    else:
                ...
                with timing.add_time("update"):
                    ...
                    loss.backward()
                    # if self.cfg.region == 1:
                    #     self.actor_critic.core.core.w_rnn.grad *= torch.Tensor(self.actor_critic.core.core.w_rnn_mask).to(self.device)
                    #     self.actor_critic.core.core.w_out.grad *= torch.Tensor(self.actor_critic.core.core.w_out_mask).to(self.device)
                    #     self.actor_critic.core.core.w_in.grad  *= torch.Tensor(self.actor_critic.core.core.w_in_mask).to(self.device)
                    if self.cfg.max_grad_norm > 0.0:
                        with timing.add_time("clip"):
                            torch.nn.utils.clip_grad_norm_(self.actor_critic.parameters(), self.cfg.max_grad_norm)
                            if self.cfg.region == 1:
                                torch.nn.utils.clip_grad_norm_(self.actor_critic.core.core.m, self.cfg.max_grad_norm/10)
