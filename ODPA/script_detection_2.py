...
CMD = "/home/jiashuncheng/code/Trasient/ODPA/model_torch.py"                 #需要执行程序的绝对路径，支持jar 如：D:\\calc.exe 或者D:\\test.jar
...
def args(i):
    _args = ["--exp", "auto_trasient_sparse_con_{}_m_{}".format(list_[i][0], list_[i][1]), "--delay", "6", "--gpu", "1", "--num_iterations", "1000", "--seed", "0", "--mode", "train", "--max_delay", "6000", "--L2", "0.1", "--transient", "True", "--regions", "False", "--connection_prob", "{}".format(list_[i][0]), "--EI", "0.8", "--learnable_m", "False", "--m", "{}".format(list_[i][1]), "--noise_h", "False"]
    return _args
 
class Auto_Run():
    def __init__(self,sleep_time,cmd):
        self.log = Logger('all_1.log',level='debug')
