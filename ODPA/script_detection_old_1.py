...
def args(i):
    _args = ["--exp", "0.3_seed_1_old_auto_trasient_sparse_con_{}_m_{}".format(list_[i][0], list_[i][1]), "--delay", "6", "--gpu", "2", "--num_iterations", "1000", "--seed", "1", "--mode", "train", "--max_delay", "6000", "--L2", "0.1", "--transient", "True", "--regions", "False", "--connection_prob", "{}".format(list_[i][0]), "--EI", "0.8", "--learnable_m", "False", "--m", "{}".format(list_[i][1]), "--noise_h", "True"]
    return _args
