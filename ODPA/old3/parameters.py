...
p.add_argument('--delay', type=str, default='3',choices=['random', '3', '6'])
...
p.add_argument('--max_delay', type=int, default=3000)
p.add_argument('--m', type=float, default=0.2)
p.add_argument('--L2', type=float, default=0.1)
p.add_argument('--hidden_num', type=int, default=600)
...
p.add_argument('--EI', type=float, default=0.8)
p.add_argument('--noise_h', type=str2bool, default=True)
p.add_argument('--learnable_m', type=str2bool, default=True)
...
if args.delay == 'random':
    delay = np.random.choice([3000,4000,5000,6000])
    temp = True
elif args.delay == '3':
    delay = 3000
    temp = False
...
assert args.max_delay >= delay
assert args.EI>=0 and args.EI<=1
...
par = {
    ...
    'exc_inh_prop'          : args.EI,    # excitatory/inhibitory ratio, set to 1 so that units are neither exc or inh
    'balance_EI'            : True,
    'connection_prob'       : args.connection_prob,   # 0.4  # 0.54

    # Network shape
    'num_motion_tuned'      : 24,
    'num_fix_tuned'         : 0,
    'num_rule_tuned'        : 0,
    'n_hidden'              : args.hidden_num, # 100
    'n_output'              : 3, # 3  odors + 4  odors + fix + match + nonmatch + delay
    'sensor_region'         : args.hidden_num//3, # 30   3/10
    'association_region'    : args.hidden_num//3, # 40   4/10
    'motor_region'          : args.hidden_num//3, # 30   3/10
    'hidden_loc'            : np.linspace(-np.pi, np.pi, args.hidden_num),
    'z_range'               : 2 * np.pi,
...
def regions_conn_2(w_rnn, sensor, association, motor, sparse=par['sparse']):   # sensor - association - motor
