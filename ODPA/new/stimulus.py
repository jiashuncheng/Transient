...
class Stimulus:
    ...
    def generate_trial(self, delay, test_mode = False, set_rule = None):
    ...
    # def generate_basic_trial(self, test_mode, set_rule = None):
        ...
        if args.delay == 'random':
            num_time_steps = (par['fix_time']+par['sample_time']+par['test_time']+args.max_delay*1000)//par['dt']
        else:
            num_time_steps = (par['fix_time']+par['sample_time']+par['test_time']+self.delay)//par['dt']
        trial_info = {'desired_output'  :  np.zeros((num_time_steps, par['batch_size'], par['n_output']),dtype=np.float32), # [250, 1024, 3]
                      ...
                      'neural_input'    :  np.random.normal(par['input_mean'], par['noise_in'], size=(num_time_steps, par['batch_size'], par['n_input']))# if args.noise_h else np.zeros((num_time_steps, par['batch_size'], par['n_input']))
                      }
        ...
        for t in range(par['batch_size']):
            ...
            """
            if par['num_rules'] == 1:
                match_rotation = int(par['num_motion_dirs']*par['rotation_match']/360)  # 2*0/360 = 0
            else:
            ...
            end_onset = (par['dead_time']+par['fix_time']+par['sample_time'] + par['delay_time'] + par['test_time'])//par['dt'] 
    ...
    def create_tuning_functions(self):
        ...
        motion_tuning = np.zeros((par['n_input'], par['num_receptive_fields'], par['num_motion_dirs'])) # [24,1,2]
        ...
        if args.mode == 'train':
            stim_dirs = np.float32(np.arange(0,360,360/par['num_motion_dirs'])) # (0,360,360/2)
        else:
            stim_dirs = np.float32(np.array([0.,float(args.theta)])) # (0,360,360/2)

        for n in range(par['num_motion_tuned']//par['num_receptive_fields']):  # 24
            for i in range(par['num_motion_dirs']):  # 2
