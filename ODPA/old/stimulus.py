...
class Stimulus:
    ...
    def generate_trial(self, delay, test_mode = False, set_rule = None):
        ...
        self.delay = delay
        # self.choose_match = match_int
        # trial_info1 = self.generate_basic_trial(1, test_mode, set_rule)
        # trial_info1['neural_input'] = np.maximum(0., trial_info1['neural_input'])

        # trial_info2 = self.generate_basic_trial(0, test_mode, set_rule)
        # trial_info2['neural_input'] = np.maximum(0., trial_info2['neural_input'])

        # trial_info3 = self.generate_basic_trial(1, test_mode, set_rule)
        # trial_info3['neural_input'] = np.maximum(0., trial_info3['neural_input']) 

        # return trial_info1, trial_info2, trial_info3
    ...
    # def generate_basic_trial(self, test_mode, set_rule = None):
        ...
        for t in range(par['batch_size']):
            ...
            test_onset = (par['dead_time']+par['fix_time']+par['sample_time'] + self.delay)//par['dt']  # 得到test的开始时间
            ...
            test_time_rng =  range(test_onset, num_time_steps) 
