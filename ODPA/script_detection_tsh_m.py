...
for x in range(1,101):
    list_.append(['{:.2f}'.format(x*0.01)])

def args(i):
    _args = ["--exp", "0125_0.01_auto_noise_trasient_sparse_con_hie{}".format(list_[i][0]), "--delay", "6", "--gpu", "0", "--num_iterations", "1000", "--seed", "0", "--mode", "train", "--transient", "True", "--connection_prob", "{}".format(list_[i][0]), "--regions", "True", "--hidden_num", "120", "--noise_h", "True", "--lr", "0.01"]
    return _args
 
class Auto_Run():
    def __init__(self,sleep_time,cmd):
        self.log = Logger('log/tsh.log',level='debug')
